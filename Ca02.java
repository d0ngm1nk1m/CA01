...
public class Ca02 {
	...
	static double decrease = 0.80;	// 젖은 배열의 칸에서 젖지 않은 배열의 칸으로 이동시 에너지 감쇄율
	static double stop = 5;	// 이 크기보다 작은 에너지를 가진배열이 있을 경우 더이상 전진 불
	static double top = 0.50;	// 정면으로 얼마만큼의 크기가 이동하는지
		...
		for(int i = 0; i < size; i++) {
			if(i == size - 1)
				for(int j = 0; j < size; j++) {
					energy = (double)(Math.random()*30)+70;	// 파도의 힘이 다 다르다고 해서 추가 최소 70 최대 100
					test[i][j] = new Type_and_State(tsunami, wet, energy);	// 객체 생성 후 test 이차원 배열에 입력
				}
			else if(i >= size - 10 && i <= size - 1) {
				for(int j = 0; j < size; j++)
					test[i][j] = new Type_and_State(water, wet, energy_zero);
			}
			else
				for(int j = 0; j < size; j++)
					test[i][j] = new Type_and_State(soil, dry, energy_zero);	// 파도가 없고 모두 에너지가 없다고 가정
		}
	...
	static void print_test(Type_and_State test[][]){ // print test[size][size]
		...
		for(int i = 0; i < size; i++) {
			for(int j = 0; j < size; j++)
				System.out.print(test[i][j].type);
			System.out.println();
		}
		System.out.println();
