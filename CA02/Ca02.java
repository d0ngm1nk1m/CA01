...
public class Ca02 {
	...
	static double decrease = 0;	// 젖은 배열의 칸에서 젖지 않은 배열의 칸으로 이동시 에너지 감쇄율
		...
		for(int i = 0; i < size; i++) {
			if(i == size - 1)
				for(int j = 0; j < size; j++) {
					energy = (double)(Math.random()*40)+80;	// 파도의 힘이 다 다르다고 해서 추가 최소 70 최대 100
	...
	static void print_test(Type_and_State test[][]){ // print test[size][size]
		...
		for(int i = 0; i < size; i++) {
			for(int j = 0; j < size; j++)
				if(test[i][j].type == 0)
					System.out.print("□ ");
				else if(test[i][j].type == 1)
					System.out.print("▤ ");
				else if(test[i][j].type == 2)
					System.out.print("■ ");
	...
	static void action(Type_and_State test[][], int place) {	// 파도가 오는 것
		...
		for(int i = place; i >= place; i--) {	
			for(int j = size - 1; j >= 0; j--) {
				decrease = (double)(Math.random()*0.1) + 0.75;
